...
class DemoTourTest(unittest.TestCase):
    ...
    def test_tour_showcases_the_spec_site(self) -> None:
                ...
                with sync_playwright() as playwright:
                    ...
                    try:
                        try:
                            ...
                            self.fail(f"the Telos tour did not complete: {error}")
                        self.assertEqual("three in a row", result.global_search_query)
                        self.assertRegex(result.intent_id, r"^INT-\d{4}$")
                        self.assertTrue(result.scenario_id.startswith("SCN-"), result.scenario_id)
                        self.assertEqual(result.intent_id, result.graph_query)
                        self.assertEqual(result.intent_id, result.selected_node)
                        self.assertEqual("Board", result.glossary_query)
                        self.assertGreaterEqual(result.glossary_consumers, 3)
                        self.assertEqual(
                            ["/", f"/intent/{result.intent_id}", "/graph", "/glossary"],
